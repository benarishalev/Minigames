...
def derive_key(passphrase):
    salt = b'some_salt'  # Salt should be unique per application
    kdf = hashlib.pbkdf2_hmac('sha256', passphrase.encode(), salt, 100000, 32)  # 32 bytes key length
...
file_location = r'C:\Users\user\Desktop\game\encrypted_data.bin'
...
def spawnSpecialGun(special_guns, player):
    ...
    if chance >= 99.9 and len(special_guns) < 1 and not player.doubleGun and not player.tripleGun:
...
class DroneEffact():
    ...
    def render(self, drone_effact_list):
        ...
        if self.radius >= self.maxRadius:
            if self.color > 40:
                ...
                drone_effact_list.remove(self)

def drawDroneEffact(drone_effact_list):
    for drone_effact in drone_effact_list:
        DroneEffact.render(drone_effact, drone_effact_list)
...
class Shield():
    ...
    def render(self, player, shields, enemies):
        if self.color > self.onTime:
            self.color -= self.onTime
        self.finalColor = (0, self.color, self.color)
...
def spawnShield(player, shields, enemies):
    ...
    if chance > 599 and not player.shield and len(shields) < 3:
...
class FoodPar():
    def __init__(self, x, y):
        ...
        self.color2 = 176
        self.color3 = 29
        self.finalColor = (self.color1, self.color2, self.color3)
    ...
    def render(self, food_par_list):
        ...
        pygame.draw.circle(screen, self.finalColor, (self.x + 15 / 2, self.y + 15 / 2), self.radius, 1)
        if self.color1 <= 50:
...
def drawUpgrade():
    upgrade_text = font.render(f"{next_up} +1", 0, (255, 255, 255))
    ...
    def render(self, player, gun):
        ...
        if self.xp != 0:
            if self.xp >= 25 * self.lvl:
                ...
                self.lvl += 1
                upgrade(player, gun)
            pygame.draw.rect(screen, self.color, (self.x + 1, self.y + 1, (self.xp * ((screen_w - 200) / (25 * self.lvl))) - 2, 20 - 2), 0, 5)
...
class Player():
    ...
    def render(self):
        ...
        if self.hp <= 0:
            ...
            if self.kills > best_kills:
                best_kills = self.kills
            HomeScreenRun = True
            GameRun = False
            saveDB()
    ...
    def PlayerDrone(self, enemies, drone_bullets, drone_effact_list):
        if self.drone:
            ...
            if self.droneHp <= 0:
                self.drone = False
                self.droneHp = 10
                drone_effact = DroneEffact(self.droneX, self.droneY)
                drone_effact_list.append(drone_effact)
...
def Game(GameRun):
    ...
    while GameRun:
        ...
        background((25, 25, 25), (50, 50, 50), 25)
        ...
        Player.PlayerDrone(player, enemies, drone_bullets, drone_effact_list)
        drawDroneEffact(drone_effact_list)
        ...
        # level / upgrades
        Level.render(lvl_bar, player, gun)
        drawUpgrade()
...
def drawItem(x, y, item, color):
    ...
    text = big_num_font.render(f"{item:.2f}", 0, color)
    screen.blit(text, (x + 7, y - 1))
...
class PlayButton():
    ...
    def render(self):
        ...
        if play_button_rect.collidepoint(mouse_pos) and mouse_press:
...
def HomeScreen(HomeScreenRun):
    ...
    while HomeScreenRun:
        ...
        background((25, 25, 25), (50, 50, 50), 50)
        ...
        drawItem(190, 20, best_kills, (128, 0, 255))
...
class HomeUpgrades():
    ...
    def click(self):
        ...
        if button.collidepoint(mouse_pos) and coins >= self.price:
            coins -= self.price
            if not self.name == 'coin multiple':
...
def UpgradeScreen(upgradeScreenRun):
    ...
    if not set_upgrades:
        ...
        speed_upgrade = HomeUpgrades(190, 100, 'speed', upgrade_prices[1], 100, 0.2)
        ...
        coin_multiple_upgrade = HomeUpgrades(20, 340, 'coins', upgrade_prices[8], 1.4, 1)
        set_upgrades = True
        upgrades_list = (
...
def load():
    global up_list, coins, best_kills, upgrade_prices
    try:
